...
const formSchema = z.object({
  clientName: z.string().trim().min(2, 'Имя должно содержать минимум 2 символа').max(100),
  phone: z.string().trim().min(6, 'Введите корректный номер телефона').max(20),
  address: z.string().trim().min(5, 'Адрес слишком короткий').max(300),
  problemText: z.string().trim().min(10, 'Опишите проблему подробнее (минимум 10 символов)').max(1000),
});
...
const ClientForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('requests').insert({
      ...
      problem_text: result.data.problemText,
    });
    setLoading(false);
      ...
      <div className="flex min-h-[80vh] items-center justify-center p-4">
        <Card className="w-full max-w-md text-center shadow-lg">
          <CardContent className="pt-10 pb-10">
            <ClipboardCheck className="mx-auto h-16 w-16 text-status-done mb-4" />
            <h2 className="text-2xl font-bold text-foreground mb-2">Заявка принята!</h2>
            <p className="text-muted-foreground mb-6">Мы свяжемся с вами в ближайшее время.</p>
            <Button onClick={() => { setSubmitted(false); setForm({ clientName: '', phone: '', address: '', problemText: '' }); }}>
              Подать ещё заявку
            </Button>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                id="problemText"
                placeholder="Опишите, что именно нужно отремонтировать..."
                rows={4}
